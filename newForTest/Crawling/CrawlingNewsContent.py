...
def getTheContent(nproxy, ntype, nfile):
    ...
    for line in nfile.readlines():
        ...
        if len(line) < 2 or line[-2] != '/' :
            continue
        if line.find('https://m.sohu.com/n') == -1:
            continue
        # else:
        #     print(line)
        ...
        if len(content) >= 1000:
            ...
            for trr in tr:
                if trr.string is not None:
                    news += trr.string
            if os.path.exists('CrawlingData/NewsContent'):
                file = open('CrawlingData/NewsContent/%s.txt' % ntype, 'a',encoding='utf-8')
                file.write(str(dict[ntype]) + ': ' + news + '\n')
            else:
                os.makedirs('CrawlingData/NewsContent')
                file = open('CrawlingData/NewsContent/%s.txt' % ntype, 'a',encoding='utf-8')
                file.write(str(dict[ntype]) + ': ' + news + '\n')
            dict[ntype] += 1
            lock.release()
